...
Use `maxResults` between 1 and 10. For broad discovery use `schemaDetail: "summary"`,
then request `schemaDetail: "input"` with the chosen exact tool name. Reuse the
schema for this connection; do not rediscover it for each item or approval. If the
schema rejects input, correct the named constraint instead of repeating the query.

For bulk existing-item modifier mapping, use `preview_menu_changes` with
