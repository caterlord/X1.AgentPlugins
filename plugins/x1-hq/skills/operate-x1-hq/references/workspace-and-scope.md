...
1. Call `bootstrap_hq_workspace` on a new authenticated connection with any
   explicit company, brand, or shop names. Retain its selected path or returned
   candidates as verified conversational context for that connection.
...
4. Let `bootstrap_hq_workspace` collapse every single-option level and persist
   the uniquely resolved workspace, company, brand, and shop defaults.
...
6. When it returns `needs_selection`, ask one concise question only at the first
   required ambiguous level. Show the returned shortest disambiguation list,
   then call the same bootstrap tool again with the user's choice.
...
8. Resolve company, brand, shop, menu, category, item, modifier, and device names
   ...
   Use `list_hq_workspace_options` plus `commit_set_hq_workspace` only as the
   compatibility fallback when the bootstrap tool is genuinely unavailable.
